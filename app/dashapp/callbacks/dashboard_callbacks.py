...
def register_dashboard_callbacks(app):
    ...
    def initialize_filters(pathname):
        ...
        if not current_user.is_authenticated:
            ...
            return [{"label": "Login Required", "value": "error"}], years, years, months, months, None
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if markets_list:
                    ...
                    markets = [{"label": market, "value": market} for market in markets_list]
                ...
                years = [{"label": str(year), "value": year} for year in range(year_min, year_max + 1)]
                ...
                years = [{"label": str(year), "value": year} for year in range(2000, 2024)]
                months = [{"label": str(month), "value": month} for month in range(1, 13)]
                return [{"label": "Error loading markets", "value": "error"}], years, years, months, months, None
    ...
    def update_dashboard(n_clicks, market, start_year, start_month, end_year, end_month):
        
        if n_clicks is None or not all([market, start_year, start_month, end_year, end_month]):
            print("Filtros incompletos, retornando gráfico vazio")
            return px.line(title=""), html.Div([
                html.H4("Por Favor, Selecione Todos os Filtros", className="text-info"),
        ...
        if market in ["none", "error"]:
            ...
            return px.line(title="Seleção de Mercado Inválida"), html.Div([
                ...
                html.P("Se nenhum mercado válido estiver disponível, o banco de dados pode não conter dados de voo.")
            ])
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if not data.get("labels") or not data.get("rpk_values") or len(data.get("labels", [])) == 0:
                        print("Nenhum dado encontrado para os filtros selecionados")
                        no_data_fig = px.line(title=f"Sem dados para o mercado {market}")
                        ...
                        return no_data_fig, html.Div([
                            html.H4("Nenhum Dado Encontrado", className="text-warning"),
                            html.P(f"Nenhum dado de voo encontrado para o mercado '{market}' no intervalo de datas selecionado."),
                            html.P(f"Intervalo de datas: {start_year}/{start_month} - {end_year}/{end_month}"),
                            html.P("Tente selecionar um mercado diferente ou intervalo de datas.")
                        ])
                    ...
                    table = dash_table.DataTable(
                        ...
                        columns=[
                            ...
                            {"name": "RPK", "id": "rpk", "type": "numeric", "format": {"specifier": ",.2f"}}
                        ],
                    ...
                    error_message = html.Div([
                        html.H4("Error parsing API response", className="text-danger"),
                ...
                return px.line(title="Autenticação Necessária"), html.Div([
                    ...
                    dcc.Location(pathname="/login", id="redirect-to-login-from-dash", refresh=True)
                ])
            else:
